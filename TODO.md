...
* Add a UI for setting the value within the WP admin (under Writing)
* Detect the number of custom fields that could be listed and dynamically adjust to show them all
    * If excessively large, replace the meta key name field with a smarter input field (akin to select2)
    * Trigger the behavior by accepting a special key as the limit value, e.g. "auto"
